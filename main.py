...
import wolframalpha
import json
client = wolframalpha.Client(os.getenv('wolframalphacode'))
...
def model():
	...
	codes = client.query('what is '+str(response))
	codes = list(codes)[2].text
	if 'ICD' in codes:
		return json.dumps([response['Body'].read().decode(),codes])
	else:
		return json.dumps([response['Body'].read().decode(),''])
